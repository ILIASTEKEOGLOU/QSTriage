...
The primary CI workflow uses read-only repository permissions, immutable full-length action commit references, non-persistent checkout credentials, bounded job execution, and dependency-consistency checks. The Python 3.11 CI environment is installed from a hashed dependency lock. A separate read-only security workflow audits both the application lock and the Ubuntu 24.04 / Python 3.11 release-tool lock, runs Bandit across runtime and release tooling, and performs full-history Gitleaks scanning. Dependabot monitors Python and GitHub Actions updates. A separate release-artifact workflow uses an exact Python 3.11 build-tool lock, proves wheel and source-distribution reproducibility across two clean source trees, and emits SHA-256 checksums and a reproducible CycloneDX SBOM. Public tag or approved main-branch manual runs also create GitHub provenance and SBOM attestations; private repository runs intentionally skip GitHub-hosted attestations while retaining the release bundle and local integrity evidence.
